...
const { Series, Nfts, User, sequelize } = require('../models');
        ...
        if (receipt.status === 1) {
            ...
            if (createUser || created) {
                const createSeries = await Series.create(
                    {
                        ...
                        ticketType: seriesInfo.ticketType,
                ...
                if (createSeries) {
                    ...
                    //console.log(nfts);
                    const creatNfts = await Nfts.bulkCreate(nfts, { transaction: seqTx });
                    if (creatNfts) {
                        res.status(200).json({
